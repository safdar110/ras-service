...
//    @Query("SELECT COALESCE(SUM(milestoneCost)/COUNT(*),0.00) as averageRevenue from Milestone")
    @Query(nativeQuery = true, value = "SELECT (SELECT COALESCE(SUM(milestone_expected_payment)/COUNT(m.id),0.00) FROM milestone m JOIN project_project_milestones pm ON m.id = pm.project_milestones_id JOIN project p ON p.id = pm.project_id where m.flag =1) as averageRevenue\n" +
            "        \n" +
            "       ")
